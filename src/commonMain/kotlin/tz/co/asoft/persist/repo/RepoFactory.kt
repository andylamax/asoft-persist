...
object RepoFactory {
    ...
    inline fun <reified T : Any, D : IDao<T>, R : IRepo<T>> getRepo(dao: D, builder: (D) -> R = { Repo(dao) as R }): R {
        return repos.getOrPut(T::class) { builder(dao) } as R
    }

    inline fun <reified T : Any, D : PaginatedDao<T>, R : PaginatedRepo<R>> getRepo(dao: D, builder: (D) -> R = { PaginatedRepo(it) as R }): R {
